-- 第2章 なぜ関数型プログラミングが重要か
-- ========

-- ## 小目次
-- <div class="toc">
-- <ul class="toc">
--   <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#features-of-functional-programming">2.2 関数型プログラミングの特徴</a>
--     <ul>
--       <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#first-class-object">ファーストクラスオブジェクトとしての関数</a></li>
--       <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#referential-transparency">参照透過性</a></li>
--     </ul>
--   </li>
--   <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#advantages-of-functional-programming">2.3 関数型プログラミングの利点</a>
--     <ul>
--       <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#modularity">高いモジュール性</a></li>
--       <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#testability">テストが容易である</a></li>
--       <li><a href="http:--akimichi.github.io/functionaljs/chap02.spec.html#provable">コードの正しさを証明できる</a></li>
--     </ul>
--   </li>
-- </ul>
-- </div>



-- ## 2.2 <section id='features-of-functional-programming'>関数型プログラミングの特徴</section>
describe('関数型プログラミングの特徴', function() 
  -- ### <section id='first-class-object'>ファーストクラスオブジェクトとしての関数</section>
  describe('ファーストクラスオブジェクトとしての関数', function()
    -- ~~~
    -- node> local zero = 0;
    -- undefined
    -- node> local birthday = {
    -- ...             year: 1999,
    -- ...             month: 1,
    -- ...             day: 12
    -- ...           };
    -- undefined
    -- node> Math.sqrt(2)   
    -- 1.4142135623730951  
    -- ~~~

    it('数値はファーストクラスオブジェクトである', function()
      --/*  #@range_begin(number_as_first_class_citizen) */
      -- 値を変数にバインドする
      local zero = 0;
      local name = "Haskell Curry";
      -- 値をデータ構造に埋めこむ
      local birthday = {
        year =  1999,
        month =  1,
        day =  12
      };
      -- 関数から値を返す
      local birthYear = function(birthdayObject)
        return birthdayObject.year;
      end;
      --/* #@range_end(number_as_first_class_citizen) */
      -- 値を関数に渡す
     assert.are.equal(math.sqrt(2), 1.4142135623730951455)
      -- expect(
      --   Math.sqrt(2)
      -- ).to.eql(
      --   -- 関数から値を返す
      --   1.4142135623730951
      -- );
    end);
    it('関数は変数にバインドできる', function()
      --/* #@range_begin(function_bound_to_variable) */
      local succ = function(n)
        return n + 1;
      end;
      --/* #@range_end(function_bound_to_variable) */
      assert.are.equal(succ(1), 2)
      -- expect(
      --   succ(1) -- 変数succを用いてλ式を呼びだす
      -- ).to.eql(
      --   2
      -- );
    end);
    it('関数をオブジェクトに埋めこむ', function()
      --/* #@range_begin(function_embedded_in_object) */
      local Math = {
        add =  function(n,m)
          return n + m;
        end 
      };
      assert.are.equal(Math.add(1, 2), 3)
      -- expect$
      --   math.add(1,2)
      -- ).to.eql(
      --   3
      -- );
      --/* #@range_end(function_embedded_in_object) */
    end);
    -- **リスト 2.1** forEach文によるsumの定義
    it('forEach文によるsumの定義', function()
      --/* #@range_begin(sum_forEach) */
      pending("I should finish this test later")
      -- local sum = function(array)
      --   local result = 0;
      --   array.forEach((item) => { -- forEachに関数を渡す
      --     result = result + item;
      --   });
      --   return result;
      -- end;
      -- --/* #@range_end(sum_forEach)  */
      -- expect(
      --   sum([1,2,3,4])
      -- ).to.eql(
      --   10
      -- );
    end);
    describe('関数を返す', function()
      -- adderを定義する
      it('adderを定義する', function()
        --/* #@range_begin(adder_definition) */
        local function adder(n)
          return function(m) -- 関数を返す
            return n + m;
          end;
        end;
        --/* #@range_end(adder_definition) */
        local succ = adder(1);
        assert.are.equal(succ(0), 1)
        -- expect(
        --   succ(0)
        -- ).to.eql(
        --   1
        -- );
      end);
    end);
  end);
  -- ### <section id='referential-transparency'>参照透過性</section>
  describe('参照透過性', function()
    -- > 参考資料: [Wikipediaの記事](https:--ja.wikipedia.org/wiki/%E5%8F%82%E7%85%A7%E9%80%8F%E9%81%8E%E6%80%A7)

    -- #### 参照透過性が成立する場面
    describe('参照透過性が成立する場面', function()
      -- 値の参照透過性
      it('値の参照透過性', function()
        assert.are.equal(2 == 2, true)
        -- expect(
        --   2 == 2
        -- ).to.eql(
        --   true
        -- );
      end);
      -- 変数の参照透過性
      describe('変数の参照透明性', function()
        it('変数が参照透明性を持つ場合', function()
          local x = 1;
          assert.are.equal(x == x, true)
          -- expect(
          --   x == x
          -- ).to.eql(
          --   x
          -- );
        end);
        it('代入は変数の参照透明性を破壊する', function()
          local foo = 2;
          local bar = foo;
          foo = 3;
          assert.are_not.equal(foo, bar)
    
          -- expect(
          --   foo
          -- ).to.not.eql(
          --   bar
          -- );
        end);
      end);
      -- 関数の参照透過性
      describe('関数の参照透明性', function()
        -- succ関数は参照透明性を持つ
        it('succ関数は参照透明性を持つ', function()
          local succ = function(x)
            return x + 1;
          end;
          -- ~~~
          -- node> succ(1) === succ(1)
          -- true
          -- ~~~
          assert.are.equal(
            succ(1) == succ(1)
          , 
            true)
          -- expect(
          --   succ(1) == succ(1)
          -- ).to.eql(
          --   true
          -- );
          assert.are.equal(
            succ(1)
          , 
            2)
          -- expect(
          --   succ(1) -- 1回目の実行
          -- ).to.eql(
          --   2
          -- );
          assert.are.equal(
            succ(1)
          , 
            2)
          -- expect(
          --   succ(1) -- 2回目の実行
          -- ).to.eql(
          --   2
          -- );
        end);
      end);
    end);
    -- #### 参照透過性を破壊するもの
    describe('参照透過性を破壊するもの', function()
      -- 可変なデータは参照透明性を破壊する
      -- ~~~
      -- node> local array = [1];
      -- node> array
      -- [ 1 ]
      -- node> array.push(2);
      -- 2
      -- node> array
      -- [ 1, 2 ]
      -- ~~~
      it('可変なデータは参照透明性を破壊する', function()
        local array = {1};
        -- expect(
        --   array
        -- ).to.eql(
        --   [1]
        -- );
        -- array.push(2);
        -- expect(
        --   array
        -- ).to.eql(
        --   [1,2]
        -- );
      end);
      -- **リスト2.2** 代入が参照透明性を破壊する例
      it('代入が参照透明性を破壊する例', function()
        --/* #@range_begin(assignment_breaks_referential_transparency) */
        local x = 0;
        local add = function(y)
          x = x + 1; -- 代入で変数を更新する
          return x + y;
        end;
        --/* #@range_end(assignment_breaks_referential_transparency)  */
        assert.are.equal(
          add(1)
        , 
          2)
        -- expect(
        --   add(1)
        -- ).to.eql(
        --   2
        -- );
      end);
      -- **リスト2.3** 命令的な階乗関数
      it('命令的な階乗関数', function()
        --/* #@range_begin(imperative_factorial) */
        local factorial = function(n)
          --/* 変数resultに結果が入る */
          local result = 1;             
          --/* 変数timesは反復の回数を数える */
          local times = 1;              
          --/* while文は反復を処理する */
          while(times < n + 1) do
            --/* 変数resultを代入で更新する */
            result = result * times;  
            --/* 変数timesを代入で更新する */
            times = times + 1;        
          end 
          return result;
        end;
        --/* #@range_end(imperative_factorial) */
        assert.are.equal(
          factorial(2)
        , 
          2)
        -- expect(
        --   factorial(2)
        -- ).to.eql(
        --   2
        -- );
        assert.are.equal(
          factorial(3)
        , 
          6)
        -- expect(
        --   factorial(3)
        -- ).to.eql(
        --   6
        -- );
        assert.are.equal(
          factorial(4)
        , 
          24)
        -- expect(
        --   factorial(4)
        -- ).to.eql(
        --   24
        -- );
      end);
      -- 副作用と入出力
      describe('副作用と入出力', function()
        -- Date.now関数は参照透明性を持たない
        it('Date.now関数は参照透明性を持たない', function()
          pending("I should finish this test later")
          -- local onceUponATime = Date.now();
          --/* 時間を1秒進める */
          -- setTimeout(() => {
          --   expect(
          --     onceUponATime
          --   ).to.not.eql( --/* 等しくないことをテストしている */
          --     Date.now()
          --   );
          -- }, 1000)
          -- local sleep = require('sleep-async')();
          -- sleep.sleep(1000, () => {
          --   expect(
          --     onceUponATime
          --   ).to.not.eql( --/* 等しくないことをテストしている */
          --     Date.now()
          --   );
          -- });
        end);
        -- ファイル入出力が参照透明性を破壊する例
        it('ファイル入出力が参照透明性を破壊する例', function()
          pending("I should finish this test later")
        --   local fs = require('fs');
        --   local read = function(path)
        --     local readValue = fs.readFileSync(path);
        --     return parseInt(readValue);
        --   end;
        --   local write = (path, n) => {
        --     fs.writeFileSync(path, n);
        --     return n;
        --   };
        --   write('test/resources/io.txt', 1);
        --   expect(
        --     read('test/resources/io.txt')
        --   ).to.eql(
        --     1
        --   );
        --   write('test/resources/io.txt', 2); -- ここでファイルに値を書きこむ
        --   expect(
        --     read('test/resources/io.txt')
        --   ).to.eql(
        --     2
        --   );
        end);
      end);
    end);
    -- #### 参照透明性を保証する
    describe('参照透明性を保証する', function()
      -- ##### 値の参照透過性を保証する（可変なデータの排除）
      describe('値の参照透過性を保証する（可変なデータの排除）', function()
        -- ** リスト2.4** 不変なデータ構造としてのオブジェクト型の実装
        it('不変なデータ構造としてのオブジェクト型の実装', function()
          --/* #@range_begin(immutable_datatype) */
          local empty =  function(_)
            return null;
          end;
          local get = function(key, obj)
            return obj(key);
          end;
          local set = function(key, value, obj)
            return function(key2)
              if(key == key2) then
                return value;
              else 
                return get(key2,obj);
              end 
            end;
          end;
          --/* #@range_end(immutable_datatype) */
        end);
      end);
      -- ##### 変数の参照透過性を保証する(代入の排除)
      describe('変数の参照透過性を保証する(代入の排除)', function()
        -- ** リスト2.5** 代入を使った足し算の定義
        it('代入を使った足し算の定義', function()
          --/* #@range_begin(imperative_addition) */
          local add = function(x,y)
            local times = 0;          
            local result = x;         

            --/* while文で反復を処理する */
            while(times < y) do
              result = result + 1;
              times = times + 1;    
            end 
            return result;
          end;
          --/* #@range_end(imperative_addition) */
          assert.are.equal(
            add(2,1)
          , 
            3)
          -- expect(add(2,1)).to.eql(3);
          assert.are.equal(
            add(2,2)
          , 
            4)
          -- expect(add(2,2)).to.eql(4);
          assert.are.equal(
            add(3,2)
          , 
            5)
          -- expect(add(3,2)).to.eql(5);
        end);
        -- ** リスト2.6** 関数型プログラミングによる足し算の定義
        it('関数型プログラミングによる足し算の定義', function()
          --/* #@range_begin(functional_addition) */
          local function add(x,y)
            if(y < 1) then
              return x;
            else
              --/* 新しい引数でadd関数を再帰的に呼び出す */
              return add(x + 1, y - 1); 
            end 
          end;
          --/* #@range_end(functional_addition) */
          assert.are.equal(
           add(2,1) 
          , 
            3)
          -- expect(add(2,1)).to.eql(3);
          assert.are.equal(
           add(2,2) 
          , 
            4)
          -- expect(add(2,2)).to.eql(4);
          assert.are.equal(
           add(3,2) 
          , 
            5)
          -- expect(add(3,2)).to.eql(5);
        end);
      end);
      -- ##### 関数の参照透過性を保証する（副作用の分離）
      describe('関数の参照透過性を保証する（副作用の分離）', function()
        -- **リスト2.7** 副作用が分離されていないコード
        it('副作用が分離されていないコード', function()
          --/* #@range_begin(age_sideeffect) */
          local age = function(birthYear)
            --/* todayは現時点の日付データ */
            local today = new Date();
            --/* getFullYear関数は日付データにもとづいて現時点の西暦を返す */
            local thisYear = today.getFullYear(); 
            return thisYear - birthYear;
          end;
          --/* #@range_end(age_sideeffect) */
        end);
        -- **リスト2.8** 副作用が分離されているコード
        it('副作用が分離されているコード', function()
          --/* #@range_begin(age_without_sideeffect) */
          pending("I should finish this test later")
          -- local age = function(birthYear, thisYear)
          --   return thisYear - birthYear;
          -- end;
          -- --/* #@range_end(age_without_sideeffect) */
          -- expect(
          --   age(1999, (new Date()).getFullYear())
          -- ).to.eql(
          --   17
          -- );
        end);
      end);
    end);
  end);
end);
-- ## 2.3 <section id='advantages-of-functional-programming'>関数型プログラミングの利点</section>
-- > 参考資料: [Why Functional Programming Matters](https:--www.google.co.jp/url?sa=t&rct=j&q=&esrc=s&source=web&cd=1&cad=rja&uact=8&ved=0ahUKEwijj-fo-8zPAhWFmpQKHWI4C7UQFggcMAA&url=https%3A%2F%2Fwww.cs.kent.ac.uk%2Fpeople%2Fstaff%2Fdat%2Fmiranda%2Fwhyfp90.pdf&usg=AFQjCNHb88MH2uDvwjCn94-VGVe6GjAM5Q&sig2=4hvyUIdaYiW5Qxc8H51Kwg)
describe('関数型プログラミングの利点', function()
  --/* 下記テストで利用される関数をあらかじめ定義しておく */
  local compose = function(f,g)
    return function(arg)
      return f(g(arg));
    end
  end;
  -- ### <section id='modularity'>高いモジュール性</section>
  describe('モジュール性とは何か', function()
    -- #### 部品の汎用性
    describe('部品の汎用性', function()
      -- **リスト2.9** sumの定義
      describe('sum関数の定義', function()
        -- ##### forEachメソッドによるsumの定義
        it('forEachメソッドによるsumの定義', function()
          --/* #@range_begin(sum_in_array_while) */
          local sum = function(array)
            --/* 結果を格納する変数 */
            local result = 0;
            --/* 反復した回数を格納する変数 */
            local index = 0; 
            while(index < array.length) do
              --/* 変数resultを代入で更新する */
              result = result + array[index];
              --/* 反復回数を更新する */
              index = index + 1;
            end 
            return result;
          end;
          --/* #@range_end(sum_in_array_while)  */
          -- expect(
          --   sum([1,2,3,4])
          -- ).to.eql(
          --   10
          -- );
          -- expect(
          --   sum([1,2,3,4])
          -- ).to.eql(
          --   10
          -- );
        end);
        -- ##### reduceメソッドによるsumの定義
        it('reduceメソッドによるsumの定義', function()
          --/* #@range_begin(sum_in_array_reduce) */
          pending("I should finish this test later")
          -- local sum = function(array)
          --   return array.reduce(--/* 第1引数に関数を渡す */
          --     (accumulator, item) => { 
          --       return accumulator + item; --/* 足し算を実行する */
          --     },0); -- 第2引数には、蓄積変数の初期値として0を渡す 
          -- end;
          -- --/* #@range_end(sum_in_array_reduce)  */
          -- expect(
          --   sum([1,2,3,4])
          -- ).to.eql(
          --   10
          -- );
        end);
      end);
      -- **リスト2.10** product関数の定義
      it('product関数の定義', function()
        --/* #@range_begin(product_in_array_reduce) */
        pending("I should finish this test later")
        -- local product = function(array)
        --   return array.reduce((accumulator, item) => {
        --     return accumulator * item; --/* かけ算を実行する */
        --   }, 1); -- 第2引数には、蓄積変数の初期値として1を渡す 
        -- end;
        -- --/* #@range_end(product_in_array_reduce)  */
        -- --/* #@range_begin(product_in_array_reduce_test) */
        -- expect(
        --   product([1,2,3,4])
        -- ).to.eql(
        --   24
        -- );
        --/* #@range_end(product_in_array_reduce_test)  */
        next();
      end);
      -- **リスト2.11** map関数の定義
      it('map関数の定義', function()
        pending("I should finish this test later")
        -- local succ = (n) => {
        --   return n + 1;
        -- };
        -- --/* #@range_begin(map_in_array_reduce) */
        -- local map = (transform) => {
        --   return (array) => {
        --     return array.reduce((accumulator, item) => {
        --       return accumulator.concat(transform(item));
        --     },[]); -- 蓄積変数の初期値として空の配列[]を指定する
        --   };
        -- };
        -- --/* #@range_end(map_in_array_reduce)  */
        -- expect(
        --   map(succ)([1,2,3])
        -- ).to.eql(
        --   [2,3,4]
        -- );
        -- expect(
        --   -- **リスト2.12** map関数のテスト
        --   --/* #@range_begin(map_in_array_reduce_test) */
        --   map(succ)([1,3,5])
        --   --/* #@range_end(map_in_array_reduce_test) */
        -- ).to.eql(
        --   --/* #@range_begin(map_in_array_reduce_test_result) */
        --   [2,4,6]
        --   --/* #@range_end(map_in_array_reduce_test_result) */
        -- );
      end);
    end);
    -- #### 部品を組み合わせる
    describe('部品を組み合わせる', function()
      -- ##### 関数適用で部品を組み合わせる
      describe('関数適用で部品を組み合わせる', function()
        pending("I should finish this test later")
        -- local map = (transform) => {
        --   return (array) => {
        --     return array.reduce((accumulator, item) => {
        --       return accumulator.concat(transform(item));
        --     },[]); -- 蓄積変数の初期値として空の配列[]を指定する
        --   };
        -- };
        -- local sum = (array) => {
        --   return array.reduce((accumulator, item) => { -- 第1引数に関数を渡す
        --     return accumulator + item;                 -- 足し算を実行する
        --   },0); -- 第2引数には、蓄積変数の初期値として0を渡す 
        -- };
        -- -- **リスト2.13** constant関数
        -- --/* #@range_begin(constant) */
        -- local constant = (any) => {
        --   return (_) => {
        --     return any;
        --   };
        -- };
        -- local alwaysOne = constant(1); 
        -- --/* #@range_end(constant) */
        -- -- **リスト2.14** map(alwaysOne)で配列の全要素を1に変える
        -- --/* #@range_begin(map_alwaysOne) */
        -- expect(
        --   map(alwaysOne)([1,2,3])
        -- ).to.eql(
        --   [1,1,1]
        -- );
        -- --/* #@range_end(map_alwaysOne) */
        -- local flip = (fun) => {
        --   return  (x) => {
        --     return (y) => {
        --       return fun(y)(x);
        --     };
        --   };
        -- };
        -- **リスト2.15** 関数適用によるlength関数の定義
        it('関数適用によるlength関数の定義', function()
          --/* #@range_begin(array_length) */
          pending("I should finish this test later")
          -- local length = (array) => {
          --   return sum(map(alwaysOne)(array));
          -- };
          -- --/* #@range_end(array_length)  */
          -- expect(
          --   length([1,2,3])
          -- ).to.eql(
          --   3
          -- );
          -- next();
        end);
        -- ##### 関数合成による処理の合成
        describe('関数合成による処理の合成', function()
          -- **リスト2.16** 関数の合成
          --/* #@range_begin(function_compose) */
          local compose = function(f,g)
            return function(arg)
              return f(g(arg));
            end;
          end;
          --/* #@range_end(function_compose)  */
          -- **リスト2.17** 関数合成によるlength関数の定義（ポイントフリースタイル）
          it('関数合成によるlength関数の定義（ポイントフリースタイル）', function()
            --/* #@range_begin(array_length_in_composition_with_point_free_style) */
          pending("I should finish this test later")
          --   local length = compose(sum,map(alwaysOne));
          --   --/* #@range_end(array_length_in_composition_with_point_free_style)  */
          --   expect(
          --     length([1,2,3])
          --   ).to.eql(
          --     3
          --   );
          -- end);
          -- **リスト2.18** 関数合成によるlength関数の定義（引数を明示したスタイル）
          it('関数合成によるlength関数の定義（引数を明示したスタイル）', function()
            --/* #@range_begin(array_length_in_composition) */
             pending("I should finish this test later")
            -- local length = (array) => { -- 引数が配列であることを明示する
            --   return compose(sum,map(alwaysOne))(array);
            -- };
            -- --/* #@range_end(array_length_in_composition)  */
            -- expect(
            --   length([1,2,3])
            -- ).to.eql(
            --   3
            -- );
          end);
        end);
      end);
      -- ##### 関数による遅延評価
      -- 
      describe('関数による遅延評価', function()
        -- 評価戦略の種類
        describe('評価戦略の種類', function()
          -- local map = (transform) => {
          --   return (array) => {
          --     return array.reduce((accumulator, item) => {
          --       return accumulator.concat(transform(item));
          --     },[]); 
          --   };
          -- };
          -- local sum = (array) => {
          --   return array.reduce((accumulator, item) => { 
          --     return accumulator + item;                
          --   },0); 
          -- };
          -- local constant = (any) => {
          --   return (_) => {
          --     return any;
          --   };
          -- };
          -- local alwaysOne = constant(1); 
          -- local length = compose(sum,map(alwaysOne));
          -- **リスト2.19** 正格評価の例
          it('正格評価の例', function()
            pending("I should finish this test later")
            -- expect(
            --   --/* #@range_begin(strict_evaluation) */
            --   length([1,1+1])
            --   --/* #@range_end(strict_evaluation) */
            -- ).to.eql(
            --   --/* #@range_begin(strict_evaluation_result) */
            --   2
            --   --/* #@range_end(strict_evaluation_result) */
            -- );
          end);
          -- **リスト2.20** 遅延評価の例
          it('遅延評価の例', function()
            pending("I should finish this test later")
            -- expect(
            --   --/* #@range_begin(lazy_evaluation) */
            --   length([1,(_) => {
            --     return 1+1; 
            --   }])
            --   --/* #@range_end(lazy_evaluation) */
            -- ).to.eql(
            --   --/* #@range_begin(lazy_evaluation_result) */
            --   2
            --   --/* #@range_end(lazy_evaluation_result) */
            -- );
          end);
        end);
        -- ##### 遅延評価によるストリーム
        describe('遅延評価によるストリーム', function()
          -- **リスト2.21** ストリームの例
          it('ストリームの例', function()
            --/* #@range_begin(stream_example) */
            local aStream = {1, function(_) -- 後尾は無名関数で表現する
              return 2;
            end};
            --/* #@range_end(stream_example) */
            assert.are.equal(
              aStream[1] -- ストリームの先頭要素を取得する
            , 
            1)
            -- expect(
            --   aStream[0] -- ストリームの先頭要素を取得する
            -- ).to.eql(
            --   1
            -- );
            assert.are.equal(
              aStream[2]() -- ストリームの先頭要素を取得する
            , 
            2)
            -- expect(
            --   aStream[1]() -- 関数適用でストリームの後尾を取り出す
            -- ).to.eql(
            --   2
            -- );
          end);
          describe('無限ストリームを作る', function()
            -- **リスト2.22** enumFrom関数 
            local succ = function(n)
              return n + 1;
            end;
            --/* #@range_begin(enumFrom) */
            local function enumFrom(n)
              return {n, function(_)  -- ストリームを返す
                return enumFrom(succ(n));
              end};
            end;
            --/* #@range_end(enumFrom) */
            -- **リスト2.23** 無限の偶数列を作る
            it('無限の偶数列を作る', function()
              --/* #@range_begin(evenStream) */
               pending("I should finish this test later")
              local evenFrom = function(n)
                return {n, function(_)
                  return evenFrom(n + 2);
                end};
              end;
              local evenStream = evenFrom(2); 
              --/* #@range_end(evenStream) */
              -- local take = function(n, astream)
              --   if(n == 1)  then
              --     return astream[0];
              --   else
              --     return [astream[0]].concat(take(n-1, astream[1]()));
              --   end 
              -- end;
              -- expect(
              --   take(3, evenStream)
              -- ).to.eql(
              --   [2,4,6]
              -- );
            end);
            -- **リスト2.24** iterate関数 
            --/* #@range_begin(stream_iterate) */
            local iterate = function(init)  -- 先頭の値を渡す
              return function(step)       -- 次の値との差を計算する関数を渡す
                return {init, function(_) -- ストリーム型を返す
                  return iterate(step(init))(step);
                end};
              end;
            end;
            --/* #@range_end(stream_iterate) */

            -- **リスト2.25** 無限ストリームの例
            it('無限の偶数列', function()
              local succ = function(n)
                return n + 1;
              end;
              --/* #@range_begin(enumFrom_by_iterate) */
              local function enumFrom(n)
                return iterate(n)(succ);
              end;
              -- 自然数列を定義する
              local naturals = enumFrom(1);
              -- 偶数列を定義する
              local twoStep = function(n)
                return n + 2;
              end;
              local evenStream = iterate(2)(twoStep);
              --/* #@range_end(enumFrom_by_iterate) */
            end);
            it('ストリームを加工する', function()
              -- **リスト2.26** ストリームのfilter関数
              --/* #@range_begin(stream_filter) */
              local function filter(predicate)
                return function(aStream)
                  --/* ストリームの先頭要素を取り出す */
                  local head = aStream[1];
                  --/* 先頭要素が条件に合致する場合 */
                  if(predicate(head) == true) then
                    return {head, function(_)
                      return filter(predicate)(aStream[2]());
                    end};
                  else
                    --/* 先頭要素が条件に合致しない場合 */
                    return filter(predicate)(aStream[2]());
                  end 
                end;
              end;
              --/* #@range_end(stream_filter) */
              -- **リスト2.27**filter関数で無限の偶数列を作る
              --/* #@range_begin(evenStream_by_filter) */
              local even = function(n)
                return (n % 2) == 0; 
              end;
              local evenStream = filter(even)(enumFrom(1));
              --/* #@range_end(evenStream_by_filter) */
              -- **リスト2.28** ストリームのelemAt関数
              --/* #@range_begin(stream_elemAt) */
              local function elemAt(n)
                return function(aStream)
                  if(n == 1) then
                    return aStream[1];
                  else
                    return elemAt(n-1)(aStream[2]());
                  end;
                end;
              end;
              --/* #@range_end(stream_elemAt) */
              assert.are.equal(
                elemAt(1)(evenStream)
              , 
                2
              ) 
              -- expect(
              --   elemAt(1)(evenStream)
              -- ).to.eql(
              --   2
              -- );
              assert.are.equal(
                elemAt(2)(evenStream)
              , 
                4
              ) 
              -- expect(
              --   elemAt(2)(evenStream)
              -- ).to.eql(
              --   4
              -- );
              -- **リスト2.29** 3番目の偶数を求める
              assert.are.equal(
                elemAt(3)(evenStream)
              , 
                6
              ) 
              -- expect(
              --   --/* #@range_begin(third_element_of_evenStream) */
              --   elemAt(3)(evenStream)
              --   --/* #@range_end(third_element_of_evenStream) */
              -- ).to.eql(
              --   --/* #@range_begin(third_element_of_evenStream_result) */
              --   6
              --   --/* #@range_end(third_element_of_evenStream_result) */
              -- );
            end);
            -- **リスト2.30** 配列のelemAt関数
            it('配列のelemAt関数', function()
              --/* #@range_begin(array_elemAt) */
              local function elemAt(n)
                return function(anArray)
                  assert(type(anArray) == "table")
                   
                  if(n == 1) then 
                    return anArray[1];
                   else
                    local tail = { select(2,  table.unpack(anArray))}
                    -- local tail = anArray.slice(1,anArray.length);
                    return elemAt(n-1)(tail);
                  end;
                end;
              end;
              --/* #@range_end(array_elemAt) */
              assert.are.equal(
               elemAt(4)({2,4,6}) 
              , 
               nil 
              ) 
              -- expect(
              --   -- **リスト2.31** 4番目の偶数を求める
              --   --/* #@range_begin(fourth_element_of_evenArray) */
              --   elemAt(4)({2,4,6})
              --   --/* #@range_end(fourth_element_of_evenArray) */
              -- ).to.eql(
              --   --/* #@range_begin(fourth_element_of_evenArray_result) */
              --   undefined
              --   --/* #@range_end(fourth_element_of_evenArray_result) */
              -- );
            end);
          end);
        end);
      end);
    end);
  end);
  -- ### <section id='testability'>テストが容易である</section>
  describe('テストが容易である', function()
    -- **リスト2.32** 単体テストの書き方の例
    it('単体テストの書き方の例', function()
      local succ = function(n)
        return n + 1;
      end;
      --/* #@range_begin(expect_example) */
      assert.are.equal(
        succ(1) -- テストしたい式を書く
      , 
        2
      ) 
      -- expect(
      --   succ(1) -- テストしたい式を書く
      -- ).to.eql(
      --   2       -- 期待する結果を書く
      -- );
      --/* #@range_end(expect_example) */
    end);
    -- **リスト2.33** 参照透過性のない関数の単体テスト
    it("参照透過性のない関数の単体テスト", function()
      --/* #@range_begin(winner_with_sideeffect) */
      local winner = function(playerL, playerR)
        if(playerR.score > playerL.score) then
          console.log(playerR.name + "が勝者です");
        elseif(playerR.score < playerL.score) then
          console.log(playerL.name + "が勝者です");
        else
          console.log("引き分けです");
        end 
      end; 
      --/* #@range_end(winner_with_sideeffect) */
      local playerA = {
        name = 'a',
        score =  10
      };
      local playerB = {
        name =  'b',
        score = 20
      };
    end);
    describe('副作用を分離する', function()
      -- **リスト2.34** winner関数の分離
      it("winner関数の分離", function()
        --/* #@range_begin(winner_without_sideeffect) */
        --/* 勝者を判定する */
        local judge = function(playerL, playerR)
          if(playerL.score > playerR.score) then
            return playerL;
          elseif(playerL.score < playerR.score) then
            return playerR;
          else 
            return nil;
          end
        end; 
        --/* 勝者を告げる文字列を生成する */
        local announce = function(winner)
          if(winner) then
            return winner.name .. "が勝者です";
          else
            return "引き分けです";
          end 
        end;
        --/* 勝者を表示する */
        local displayWinner = function(winner)
          console.log(announce(winner));
        end;
        --/* #@range_end(winner_without_sideeffect) */
        -- **リスト2.35** 副作用のない関数のテスト
        --/* #@range_begin(announce_winner) */
        local socrates = {
          name = 'ソクラテス',
          score =  10
        };
        local plato = {
          name = 'プラトン',
          score = 20
        };
        --/* 純粋な関数をテストする */
        assert.are.equal(
          announce(judge(socrates, plato))
        , 
          "プラトンが勝者です"
        ) 
        
        -- expect(
        --   announce(judge(socrates, plato))
        -- ).to.eql(
        --   "プラトンが勝者です"
        -- );
        --/* #@range_end(announce_winner) */
      end);
    end);
  end);
  -- ### <section id='provable'>コードの正しさを証明できる</section>
  describe('コードの正しさを証明できる', function()
    -- #### プロパティテストで正しさを検証する
    describe('プロパティテストで正しさを検証する', function()
      local adder = function(m)
        return function(n)
          return m + n;
        end 
      end;
      local succ = adder(1);
      local function iterate(step)
        return function(init)
          return {init, function(_)
            return iterate(step)(step(init));
          end};
        end
      end;
      local enumFrom = function(from)
        return iterate(succ)(from);
      end;
      it('succ関数の性質テスト', function()
        -- **リスト2.39** プロパティテストのための関数
        --/* #@range_begin(succ_property) */
        --/* ストリームのmap関数 */
        local function map(transform)
          return function(aStream)
            local head = aStream[1];
            return {transform(head), function(_)
              return map(transform)(aStream[2]());
            end};
          end 
        end;
        --/* ストリームの先頭から引数n分だけ取り出すtake関数 */
        local function take(n)
          return function(aStream)
            if(n == 0) then
              return nil;
            else
              return {aStream[1], function(_)
                return take(n-1)(aStream[2]());
              end};
            end 
          end;
        end;
        --/* ストリームの全ての要素がtrueであるかを判定するall関数 */
        local all = function(aStream)
          local function allHelper(aStream, accumulator)
            local head = aStream[1];
            local newAccumulator = accumulator and head;
            if(aStream[2]() == nil) then
              return newAccumulator;
            else
              return allHelper(aStream[2](), newAccumulator);
            end 
          end;
          return allHelper(aStream, true);
        end;
        --/* 検証の対象となる命題 */
        local proposition = function(n)
          return succ(0) + succ(n) == succ(succ(n));
        end;
        --/* #@range_end(succ_property) */

        -- **リスト2.40** succ関数のプロパティテスト
        --/* #@range_begin(succ_property_test) */
        --/* 100個の整数について命題が正しいかをテストする */
        assert.are.equal(
          all(
            take(100)(
              map(proposition)(enumFrom(0))
            )
          )
        , 
          true
        )
        -- expect(
        --   all(
        --     take(100)(
        --       map(proposition)(enumFrom(0))
        --     )
        --   )
        -- ).to.eql(
        --   true
        -- );
        --/* #@range_end(succ_property_test) */
      end);
    end);
  end);
end);
end);

-- [目次に戻る](index.html) [次章に移る](chap03.spec.html) 
